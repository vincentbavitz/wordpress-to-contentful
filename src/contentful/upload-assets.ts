...
const uploadAssets = (client: any, assets: any, observer = MOCK_OBSERVER) =>
...
function transformForUpload(asset: any) {
  return {
    fields: {
      title: {
...
function transformForSaving(wp: any, cf: any) {
  return {
    wordpress: wp,
    contentful: {
      id: cf.sys.id,
...
async function uploadListOfAssets(client: any, observer: any) {
  const loc = path.join(ASSET_DIR_LIST, "assets.json");
  const assets = await fs.readJson(loc);
  const { done, failed } = (await uploadAssets(
    client,
    assets,
    observer
  )) as any;
...
export default (client: any) =>
  new Observable(
    (observer: any) =>
      uploadListOfAssets(client, observer).then(() =>
        observer.complete()
      ) as any
  );
